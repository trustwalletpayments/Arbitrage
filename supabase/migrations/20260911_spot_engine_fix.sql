-- Security/accounting hardening for the testnet spot RPC.
...
begin
  if auth.uid() is null or auth.uid() <> p_user_id then raise exception 'Authentication required'; end if;
  ...
  if p_client_order_id is not null then
    ...
    if found then return jsonb_build_object('order',to_jsonb(v_existing),'replayed',true); end if;
  end if;
  ...
  values(v_order.id,p_user_id,p_market,p_side,p_price,p_quantity,v_fee,v_market.quote_asset)
  returning id into v_trade_id;

  insert into public.ledger_entries(user_id,asset,wallet,amount,reference_type,reference_id)
  values
   (p_user_id,v_market.base_asset,'SPOT',case when p_side='BUY' then p_quantity else -p_quantity end,'TRADE',v_order.id),
   (p_user_id,v_market.quote_asset,'SPOT',case when p_side='BUY' then -v_cost else v_cost end,'TRADE',v_order.id),
