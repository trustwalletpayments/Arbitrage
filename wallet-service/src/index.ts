...
async function getOrAllocateEvmIndex(userId: string): Promise<number> {
  ...
  if (existing.data?.derivation_index !== null && existing.data?.derivation_index !== undefined) {
    return Number(existing.data.derivation_index);
  }
...
function deriveEvmAddress(index: number) {
  ...
  // EVM_XPUB must be an account-level xpub (normally m/44'/60'/0').
  // Only non-hardened child paths are derived from the public key.
...
app.post('/provision/:userId', authorized, async (req, res) => {
  ...
  if (!networkConfig) return res.status(400).json({ error: 'Unsupported EVM network' });
  ...
  try {
    ...
    if (inserted.error) {
      // A concurrent request can race the unique user/network/asset index.
    ...
    return res.status(500).json({ error: error instanceof Error ? error.message : 'Unable to provision wallet' });
  }
});
...
app.post('/verify-deposit', authorized, async (req, res) => {
  ...
  try {
    ...
    for (const log of receipt.logs) {
      if (log.address.toLowerCase() !== usdtContract.toLowerCase() || log.topics[0]?.toLowerCase() !== '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a9df523b3ef') continue;
    ...
    const inserted = await supabase.from('wallet_deposits').insert({ user_id: userId, wallet_account_id: account.data?.id || null, asset: 'USDT', network: 'BEP20', chain_family: 'evm', tx_hash: txHash, from_address: verifiedFrom, to_address: treasuryAddress, amount: formatUnits(verifiedAmount, 18), confirmations, status, credited_at: status === 'credited' ? new Date().toISOString() : null }).select('*').single();
