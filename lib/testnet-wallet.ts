...
export type TestnetTransaction = { id:string; type:"TRADE"|"TRANSFER"|"DEPOSIT"|"WITHDRAWAL"; asset:string; amount:number; wallet:"SPOT"|"FUTURES"; side?:"BUY"|"SELL"; symbol?:string; status:"COMPLETED"|"PENDING"; createdAt:string; note?:string };

const BALANCES_KEY="exchange:testnet:balances";
const TX_KEY="exchange:testnet:transactions";
...
export function getTestnetBalances(){return read<TestnetWallet[]>(BALANCES_KEY,DEFAULTS)}
...
export function resetTestnetWallet(){setTestnetBalances(DEFAULTS);write(TX_KEY,[])}
export function executeSpotTestnetTrade(input:{asset:string;side:"BUY"|"SELL";price:number;quantity:number;symbol:string}){
 const balances=getTestnetBalances();const quote=balances.find(x=>x.asset==="USDT");const base=balances.find(x=>x.asset===input.asset);if(!quote||!base)throw new Error("Asset is not supported");
