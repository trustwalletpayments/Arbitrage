...
export default function ProfilePage() {
  ...
  return (
    <main className={`profile-page ${theme === "light" ? "theme-light" : ""}`}>
      <header className="profile-header"><Link href="/dashboard" className="back-link"><ArrowLeft size={18} /> Back to dashboard</Link><Link href="/dashboard" className="profile-brand"><span className="brand-mark">◉</span> ORBITEX<span className="brand-dot">.</span></Link><button className="header-logout" onClick={logout}><LogOut size={17} /> Log out</button></header>
      <div className="profile-layout">
        <aside className="profile-sidebar"><div className="profile-avatar">{(email.split("@")[0] || "U").slice(0, 1).toUpperCase()}</div><strong>{email || "Your account"}</strong><span className="account-status"><Check size={13} /> Account active</span><div className="account-meta"><span>UID</span><strong>ORB-{(email || "USER").length}82931</strong></div><nav><a className="active" href="#account">Account</a><a href="#security">Security</a><a href="#referral">Referral program</a><a href="#preferences">Preferences</a><a href="#notifications">Notifications</a></nav></aside>
        <section className="profile-content">
          ...
          <section className="settings-card" id="security"><div className="card-heading"><div><span className="card-kicker">PROTECTION</span><h2>Security</h2><p>Protect your account and review active access.</p></div><ShieldCheck size={21} /></div><div className="setting-row"><div className="setting-icon"><KeyRound size={19} /></div><div className="setting-copy"><span>Password</span><strong>Keep your password private and unique</strong></div><button className="outline-button" onClick={() => setShowPassword(!showPassword)}>{showPassword ? "Cancel" : "Change"}</button></div>{showPassword && <div className="inline-editor"><input value={newPassword} onChange={(e) => setNewPassword(e.target.value)} type="password" placeholder="New password" /><button onClick={updatePassword}>Update password</button></div>}<div className="setting-row"><div className="setting-icon"><Smartphone size={19} /></div><div className="setting-copy"><span>Two-factor authentication</span><strong>Extra protection for account access</strong></div><button className="outline-button" onClick={() => setNotice("Two-factor authentication setup can be connected to your authenticator app.")}>Set up</button></div><div className="setting-row"><div className="setting-icon"><Monitor size={19} /></div><div className="setting-copy"><span>Active sessions</span><strong>Review browsers and signed-in devices</strong></div><button className="outline-button" onClick={() => setNotice("Session management will let you review and sign out of other devices.")}>Manage</button></div></section>
          <section className="settings-card referral-card" id="referral"><div className="card-heading"><div><span className="card-kicker">REWARDS</span><h2>Referral program</h2><p>Invite friends and earn rewards for eligible referrals.</p></div><QrCode size={21} /></div><div className="referral-grid"><div className="referral-copy"><div className="reward-box"><strong>$100</strong><span>Referral reward</span></div><p>Share your personal referral link with friends and track your referral activity from your account.</p><div className="ref-field"><label>Referral ID</label><div><span>{referralId}</span><button onClick={() => copy(referralId, "id")}><Copy size={16} />{copied === "id" ? "Copied" : "Copy"}</button></div></div><div className="ref-field"><label>Referral link</label><div><span>{referralLink}</span><button onClick={() => copy(referralLink, "link")}><Copy size={16} />{copied === "link" ? "Copied" : "Copy"}</button></div></div></div><div className="referral-poster"><img src={qrUrl} alt="Referral QR code" /><strong>Invite friends. Earn rewards.</strong><span>Scan to join ORBITEX</span></div></div></section>
          <section className="settings-card" id="preferences"><div className="card-heading"><div><span className="card-kicker">CUSTOMIZE</span><h2>Preferences</h2><p>Choose how Orbitex looks and displays information.</p></div><Globe size={21} /></div><div className="preference-grid"><label><span>Language</span><select value={language} onChange={(e) => setLanguage(e.target.value)}><option>English</option><option>Hindi</option><option>Spanish</option></select></label><label><span>Display currency</span><select value={currency} onChange={(e) => setCurrency(e.target.value)}><option>USD</option><option>EUR</option><option>INR</option><option>GBP</option></select></label></div><div className="theme-preference"><div className="setting-icon">{theme === "dark" ? <Moon size={19} /> : <Sun size={19} />}</div><div className="setting-copy"><span>Appearance</span><strong>{theme === "dark" ? "Dark theme selected" : "Light theme selected"}</strong></div><div className="theme-switcher compact"><button className={theme === "dark" ? "selected" : ""} onClick={() => setTheme("dark")}><Moon size={15} /></button><button className={theme === "light" ? "selected" : ""} onClick={() => setTheme("light")}><Sun size={15} /></button></div></div></section>
