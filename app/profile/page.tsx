...
import { ArrowLeft, Bell, Check, Copy, Globe, KeyRound, LogOut, Mail, Monitor, Moon, Phone, QrCode, ShieldCheck, Smartphone, Sun, Trash2, UserRound, X } from "lucide-react";
import { createSupabaseBrowserClient } from "../../lib/supabase-browser";
import "./profile.css";
import "./theme.css";

type Theme = "dark" | "light" | "system";
type NotificationSettings = { security: boolean; orders: boolean; deposits: boolean; marketing: boolean };
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const savedCurrency = window.localStorage.getItem("orbitex-currency");
  ...
  useEffect(() => { window.localStorage.setItem("orbitex-language", language); }, [language]);
  useEffect(() => { window.localStorage.setItem("orbitex-currency", currency); }, [currency]);
  ...
  return (
    <main className={`profile-page ${theme === "light" ? "theme-light" : ""} ${theme === "system" ? "theme-system" : ""}`}>
      <header className="profile-header"><Link href="/dashboard" className="back-link"><ArrowLeft size={18} /> Back to dashboard</Link><button className="header-logout" onClick={logout}><LogOut size={17} /> Log out</button></header>
      <div className="profile-layout">
        <aside className="profile-sidebar"><div className="profile-avatar">{displayName.slice(0, 1).toUpperCase()}</div><strong>{email || "Your account"}</strong><span className="account-status"><Check size={13} /> Account active</span><div className="account-meta"><span>UID</span><strong>{uid}</strong><button onClick={() => copy(uid, "uid")} aria-label="Copy UID"><Copy size={14} /></button></div><nav><a className="active" href="#account">Account</a><a href="#security">Security</a><a href="#preferences">Preferences</a><a href="#notifications">Notifications</a><a href="#referral">Referral program</a></nav></aside>
        <section className="profile-content">
          <div className="profile-title"><div><span className="eyebrow">USER PROFILE</span><h1>Account settings</h1><p>Manage your account, security, preferences and referral rewards.</p></div></div>
          ...
          <section className="settings-card" id="preferences"><div className="card-heading"><div><span className="card-kicker">CUSTOMIZE</span><h2>Preferences</h2><p>Choose your language, display currency and website appearance.</p></div><Globe size={21} /></div><div className="preference-grid"><label><span>Language</span><select value={language} onChange={(e) => setLanguage(e.target.value)}><option>English</option><option>Hindi</option><option>Spanish</option><option>French</option><option>German</option><option>Portuguese</option><option>Italian</option><option>Russian</option><option>Turkish</option><option>Arabic</option><option>Bengali</option><option>Indonesian</option><option>Vietnamese</option><option>Japanese</option><option>Korean</option><option>Chinese</option></select></label><label><span>Display currency</span><select value={currency} onChange={(e) => setCurrency(e.target.value)}><option>USD</option><option>EUR</option><option>GBP</option><option>INR</option><option>JPY</option><option>CNY</option><option>KRW</option><option>AUD</option><option>CAD</option><option>CHF</option><option>SGD</option><option>HKD</option><option>BRL</option><option>TRY</option><option>AED</option><option>SAR</option><option>RUB</option><option>MXN</option></select></label></div><div className="theme-preference"><div className="setting-icon">{theme === "light" ? <Sun size={19} /> : theme === "system" ? <Monitor size={19} /> : <Moon size={19} />}</div><div className="setting-copy"><span>Appearance</span><strong>{theme === "system" ? "Follow device theme" : `${theme === "dark" ? "Dark" : "Light"} theme selected`}</strong></div><div className="theme-switcher compact"><button className={theme === "dark" ? "selected" : ""} onClick={() => setTheme("dark")}><Moon size={15} /></button><button className={theme === "light" ? "selected" : ""} onClick={() => setTheme("light")}><Sun size={15} /></button><button className={theme === "system" ? "selected" : ""} onClick={() => setTheme("system")}><Monitor size={15} /></button></div></div></section>
