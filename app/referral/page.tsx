...
export default function ReferralPage() {
  ...
  return (
    <main className="program-page"><div className="program-shell referral-shell">
      <Link href="/dashboard" className="program-back"><ArrowLeft size={16} /> Back to dashboard</Link>
      <div className="program-icon"><Users size={28} /></div><div className="program-eyebrow">ORBITEX COMMUNITY</div>
      <h1>Referral Program</h1><p className="program-lead">Invite friends, help them get started, and earn rewards when they complete qualifying deposits.</p>
      <section className="referral-hero-grid"><div className="referral-rate-card"><div className="rate-card-icon"><Gift size={20} /></div><div className="rate-card-label">Your referral reward</div><div className="rate-card-value">{REFERRER_RATE}%</div><p>of a referred user’s qualifying deposit</p></div><div className="referral-rate-card referral-rate-card-accent"><div className="rate-card-icon"><Wallet size={20} /></div><div className="rate-card-label">Friend’s deposit bonus</div><div className="rate-card-value">{FRIEND_RATE}%</div><p>credited as a promotional bonus after eligibility checks</p></div></section>
      <section className="program-card referral-link-card"><div className="program-card-title"><Link2 size={20} /><span>Your referral link</span></div><div className="referral-link"><span>{link}</span><button type="button" onClick={copyLink} aria-label="Copy referral link">{copied ? <Check size={17} /> : <Copy size={17} />}</button></div>{copied && <div className="program-success">Referral link copied.</div>}<div className="program-code">Referral code: <strong>{code}</strong></div><div className="referral-share-row"><button type="button" className="share-button" onClick={copyLink}><Copy size={15} /> Copy link</button><a className="share-button" href={`https://t.me/share/url?url=${encodeURIComponent(link)}`} target="_blank" rel="noreferrer"><Send size={15} /> Telegram</a><a className="share-button" href={`https://wa.me/?text=${encodeURIComponent(`Join Orbitex using my referral link: ${link}`)}`} target="_blank" rel="noreferrer"><MessageCircle size={15} /> WhatsApp</a></div></section>
      ...
      <section className="program-card referral-earnings-card"><div className="program-card-title"><Wallet size={20} /><span>Reward balance</span></div><div className="earnings-row"><span>Locked rewards</span><strong>{money(locked)}</strong></div><div className="earnings-row"><span>Eligible to claim</span><strong>{money(claimable)}</strong></div><div className="earnings-row"><span>Pending review</span><strong>{money(pendingRewards)}</strong></div><button type="button" className="claim-button" disabled={claimable <= 0 || claiming} onClick={requestClaim}>{claiming ? "Submitting…" : claimable > 0 ? "Request claim" : "Not eligible yet"}</button>{message && <div className="program-success">{message}</div>}<p className="program-muted">Referral rewards unlock after at least {REQUIRED_REFERRALS} qualifying referrals. Claims are reviewed manually and paid by the Orbitex team.</p></section>
      ...
      <section className="program-card referral-history-card"><div className="program-card-title"><Users size={20} /><span>Referral history</span></div>{referrals.length === 0 ? <div className="empty-state">Your referral activity will appear here after someone joins through your link.</div> : <div className="referral-history-list">{referrals.map((referral) => <div className="earnings-row" key={referral.id}><span>{new Date(referral.created_at).toLocaleDateString()} · {statusLabel(referral.status)} · Deposit {money(Number(referral.qualifying_deposit_amount || 0))}</span><strong>{money(Number(referral.referrer_reward_amount || 0))}</strong></div>)}</div>}</section>
      <section className="program-card referral-history-card"><div className="program-card-title"><Wallet size={20} /><span>Claim history</span></div>{claims.length === 0 ? <div className="empty-state">Submitted reward claims will appear here.</div> : <div className="referral-history-list">{claims.map((claim, index) => <div className="earnings-row" key={`${claim.submitted_at}-${index}`}><span>{new Date(claim.submitted_at).toLocaleDateString()} · {statusLabel(claim.status)}</span><strong>{money(Number(claim.amount || 0))}</strong></div>)}</div>}</section>
