...
type MarketItem={symbol:string;baseAsset:string;quoteAsset:string};
...
function validPair(value:string|null):string{
  ...
  if(!/^[A-Z0-9]{2,20}$/.test(base))return "BTC/USDT";
  return `${base}/USDT`;
}
...
export default function Trade(){
    ...
    fetch("https://api.binance.com/api/v3/exchangeInfo",{cache:"no-store"})
      ...
      .then(data=>{
        ...
        const list=(Array.isArray(data?.symbols)?data.symbols:[])
          .filter((item:any)=>item.quoteAsset==="USDT"&&item.status==="TRADING"&&item.isSpotTradingAllowed!==false)
          .map((item:any)=>({symbol:item.symbol,baseAsset:item.baseAsset,quoteAsset:item.quoteAsset}))
          .sort((a:MarketItem,b:MarketItem)=>a.baseAsset.localeCompare(b.baseAsset));
        setMarkets(list);
      })
      .catch(()=>setMarkets([]));
    return()=>{alive=false};
  ...
  const filteredMarkets=useMemo(()=>{
    ...
    return markets.filter(item=>item.symbol.includes(query)||item.baseAsset.includes(query));
  },[markets,search]);
  ...
  useEffect(()=>{
    ...
    fetch(`https://api.binance.com/api/v3/ticker/24hr?symbol=${symbol}`)
      .then(r=>r.json())
      .then(r=>{if(alive&&r.lastPrice){const p=Number(r.lastPrice);setLivePrice(p);setPrice(String(p));setChange(Number(r.priceChangePercent||0));}})
      .catch(()=>{});
