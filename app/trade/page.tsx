...
export default function SpotPage(){
  ...
  async function submit(orderSide:"Buy"|"Sell",rawQuantity:string){setMessage("");const qty=Number(rawQuantity);if(!Number.isFinite(qty)||qty<=0){setMessage("Enter a valid "+base+" amount.");return}if(type==="Limit"&&(!Number.isFinite(orderPrice)||orderPrice<=0)){setMessage("Enter a valid limit price.");return}setSubmitting(orderSide);try{const response=await fetch("/api/binance/spot-order",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({symbol,side:orderSide,type,quantity:qty,price:orderPrice})});const data=await response.json();if(!response.ok||!data.ok)throw new Error(data.error||"Order was rejected.");setMessage(orderSide+" order submitted successfully. Binance order ID: "+(data.order?.orderId||"—"));if(orderSide==="Buy"){setBuyAmount("");setBuyPercent(0)}else{setSellAmount("");setSellPercent(0)}}catch(error){setMessage(error instanceof Error?error.message:"Unable to submit order")}finally{setSubmitting(null)}}
    ...
    <div className="trade-layout spot-reference-layout">
      ...
      <section className="chart-area">
        ...
        <section className="order-panel spot-order-entry">
          ...
          <div className="spot-order-types">{["Limit","Market"].map(item=><button type="button" key={item} onClick={()=>setType(item as "Limit"|"Market")} className={type===item?"active":""}>{item}</button>)}</div>
          <div className="spot-order-columns">
            <div className="spot-side-form buy-form">
              ...
              <label className="spot-field">Price <span>USDT</span><div className="spot-input-wrap"><input value={type==="Market"?(livePrice?formatPrice(livePrice):"Market price"):price} onChange={e=>setPrice(e.target.value)} readOnly={type==="Market"} placeholder="Price" inputMode="decimal"/><em>USDT</em></div></label>
