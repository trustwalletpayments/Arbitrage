...
export default function SpotPage(){
  ...
  useEffect(()=>{let alive=true;fetch(`https://api.binance.com/api/v3/ticker/24hr?symbol=${encodeURIComponent(symbol)}`,{cache:"no-store"}).then(r=>r.json()).then(data=>{if(!alive||!data.lastPrice)return;const p=Number(data.lastPrice);setLivePrice(p);setPrice(String(p));setChange(Number(data.priceChangePercent||0));setHigh(Number(data.highPrice||0));setLow(Number(data.lowPrice||0));setVolume(Number(data.v||0))}).catch(()=>{});let socket:WebSocket|undefined;try{socket=new WebSocket(`wss://stream.binance.com:9443/ws/${symbol.toLowerCase()}@ticker`);socket.onmessage=e=>{try{const data=JSON.parse(e.data);if(!alive||!data.c)return;const nextPrice=Number(data.c),nextChange=Number(data.P||0),nextHigh=Number(data.h||0),nextLow=Number(data.l||0),nextVolume=Number(data.v||0);if(tickerUpdateRef.current)return;tickerUpdateRef.current=window.setTimeout(()=>{tickerUpdateRef.current=0;if(!alive)return;setLivePrice(nextPrice);setChange(nextChange);setHigh(nextHigh);setLow(nextLow);setVolume(nextVolume)},180)}catch{}}}catch{}return()=>{alive=false;socket?.close();if(tickerUpdateRef.current)window.clearTimeout(tickerUpdateRef.current);tickerUpdateRef.current=0}},[symbol]);
  useEffect(()=>{const supabase=createSupabaseBrowserClient();supabase.auth.getUser().then(async({data:{user}})=>{if(!user){setAvailable(0);setAvailableBase(0);return}const {data}=await supabase.from("account_balances").select("asset,available").eq("user_id",user.id).eq("wallet","FUNDING").in("asset",["USDT",base]);const rows=Array.isArray(data)?data:[];setAvailable(Number(rows.find((x:any)=>x.asset==="USDT")?.available||0));setAvailableBase(Number(rows.find((x:any)=>x.asset===base)?.available||0))}).catch(()=>{setAvailable(0);setAvailableBase(0)})},[base]);
  ...
  const sellAvailableUsdt=availableBase*livePrice;const orderPrice=type==="Market"?livePrice:Number(price);const buyTotal=useMemo(()=>{const qty=Number(buyAmount);return orderPrice>0&&qty>0?qty*orderPrice:0},[orderPrice,buyAmount]);const sellTotal=useMemo(()=>{const qty=Number(sellAmount);return orderPrice>0&&qty>0?qty*orderPrice:0},[orderPrice,sellAmount]);
  function setBuyFromPercent(pct:number){setBuyPercent(pct);setBuyAmount(orderPrice>0?(available*pct/100/orderPrice).toFixed(8):"")}function setSellFromPercent(pct:number){setSellPercent(pct);setSellAmount((availableBase*pct/100).toFixed(8))}
  ...
  async function submit(orderSide:"Buy"|"Sell",rawQuantity:string){
    ...
    if(type==="Trigger Order"&&(!Number.isFinite(orderPrice)||orderPrice<=0||!Number.isFinite(Number(stopPrice))||Number(stopPrice)<=0)){setMessage("Enter a valid stop price and limit price.");return}
    ...
    <div className="trade-layout spot-reference-layout">
      ...
      <section className="chart-area">
        ...
        <section className="order-panel spot-order-entry">
          ...
          <div className="spot-order-types">{["Limit","Market","Trigger Order"].map(item=><button type="button" key={item} onClick={()=>setType(item as "Limit"|"Market"|"Trigger Order")} className={type===item?"active":""}>{item}</button>)}</div>
          <div className="spot-order-columns">
            ...
            <div className="spot-side-form sell-form">
              <div className="spot-side-head"><div><span>SELL</span><strong>{base}</strong></div><small>Available <b>{sellAvailableUsdt.toFixed(2)} USDT</b></small></div>
              <>{type==="Trigger Order"&&<label className="spot-field">Stop Price <span>USDT</span><div className="spot-input-wrap"><input value={stopPrice} onChange={e=>setStopPrice(e.target.value)} placeholder="Stop price" inputMode="decimal"/><em>USDT</em></div></label>}<label className="spot-field">{type==="Trigger Order"?"Limit Price":"Price"} <span>USDT</span><div className="spot-input-wrap"><input value={type==="Market"?(livePrice?formatPrice(livePrice):"Market price"):price} onChange={e=>setPrice(e.target.value)} readOnly={type==="Market"} placeholder={type==="Trigger Order"?"Limit price":"Price"} inputMode="decimal"/><em>USDT</em></div></label></>
