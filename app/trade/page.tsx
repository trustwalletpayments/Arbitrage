  ...
  return <main className="app-shell">
    ...
    <div className="trade-layout spot-reference-layout">
      ...
      <aside className={`market-list reference-market-list ${expanded?"expanded":""}`}>
        ...
        <div className="market-scroll">{filteredMarkets.map(item=>{const selected=displayPair(item.symbol)===pair;return <button key={`${item.symbol}-${item.marketCapRank}`} className={selected?"selected":""} onClick={()=>selectMarket(displayPair(item.symbol))}><span className="market-name">{item.logo?<img src={item.logo} alt="" width={28} height={28}/>:<CoinIcon symbol={item.baseAsset} size={28}/>}<b>{item.baseAsset}/USDT</b></span><span className="market-last">—</span><span className="market-change">—</span></button>})}</div>
