    ...
    const loadLiveMarkets = async () => {
      try {
        ...
        const liveMarkets: Market[] = data.map((item: { symbol: string; lastPrice: string; priceChangePercent: string }) => ({
          symbol: item.symbol.replace("USDT", ""), price: Number(item.lastPrice), change: Number(item.priceChangePercent),
        }));
        if (liveMarkets.length) setMarkets(liveMarkets);
      } catch { /* Keep the last successful prices visible. */ }
    };
    loadLiveMarkets();
    ...
    <main className="member-dashboard">
      ...
      <div className="member-main">
        ...
        <section className="quick-actions" aria-label="Account actions">
          <Link href="/wallet/deposit" className="quick-action"><span className="quick-action-icon"><ArrowDown aria-hidden="true" /></span><b>Add funds</b></Link>
          <Link href="/wallet/send" className="quick-action"><span className="quick-action-icon"><ArrowUp aria-hidden="true" /></span><b>Send</b></Link>
          <Link href="/wallet/transfer" className="quick-action"><span className="quick-action-icon"><ArrowLeftRight aria-hidden="true" /></span><b>Transfer</b></Link>
