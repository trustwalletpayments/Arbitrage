...
export async function POST(request: NextRequest) {
  if (!supabaseUrl || !serviceRoleKey || !walletServiceUrl || !walletServiceKey) {
    return NextResponse.json({ error: "Wallet verification is not configured." }, { status: 503 });
  }
  ...
  if (!Number.isFinite(amount) || amount <= 0 || !/^0x[a-f0-9]{64}$/.test(txHash)) {
    return NextResponse.json({ error: "Enter a valid amount and transaction hash." }, { status: 400 });
  }
