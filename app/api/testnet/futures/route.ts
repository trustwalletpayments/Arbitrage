...
export async function GET(request:Request){
  ...
  const providers=[
    `https://fapi.binance.com/fapi/v1/premiumIndex?symbol=${encodeURIComponent(safeSymbol)}`,
    `https://fapi.binance.com/fapi/v1/ticker/price?symbol=${encodeURIComponent(safeSymbol)}`,
    `https://api.binance.com/api/v3/ticker/price?symbol=${encodeURIComponent(safeSymbol)}`
  ];
  for(const url of providers){
    try{
      ...
      if(Number.isFinite(price)&&price>0)return NextResponse.json({ok:true,symbol:safeSymbol,markPrice:price});
    }catch{}
  }
