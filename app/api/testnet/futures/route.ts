...
async function getDepth(symbol:string){
  for(const path of [
    `/fapi/v1/depth?symbol=${encodeURIComponent(symbol)}&limit=12`,
    `/fapi/v1/depth?symbol=${encodeURIComponent(symbol)}&limit=5`
  ]){try{return await getJson(path)}catch{}}
  for(const path of [
    `/api/v3/depth?symbol=${encodeURIComponent(symbol)}&limit=12`,
    `/api/v3/depth?symbol=${encodeURIComponent(symbol)}&limit=5`
  ...
  try{
    ...
    if(Array.isArray(data?.b)&&Array.isArray(data?.a))return {bids:data.b,asks:data.a};
  }catch{}
  // Render at least the best bid/ask when a full depth endpoint is unavailable.
  for(const [kind,path] of [
    ["futures",`/fapi/v1/ticker/bookTicker?symbol=${encodeURIComponent(symbol)}`],
    ["spot",`/api/v3/ticker/bookTicker?symbol=${encodeURIComponent(symbol)}`]
  ] as const){
    try{
      const data=kind==="futures"?await getJson(path):await getPublicSpotJson(path);
      if(data?.bidPrice&&data?.askPrice){
        return {bids:[[String(data.bidPrice),String(data.bidQty||"0")]],asks:[[String(data.askPrice),String(data.askQty||"0")]]};
      }
    }catch{}
  }
  return {bids:[],asks:[]};
...
async function getRecentTrades(symbol:string){
  ...
  try{
    ...
    return Array.isArray(data?.list)?data.list.map((trade:any)=>({p:trade.price,q:trade.size,T:Number(trade.time||Date.now()),isBuyerMaker:trade.side!=="Buy"})):[];
  }catch{return []}
}
