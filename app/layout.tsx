...
const enhanceScript = `(() => {
  const enhancePage = () => {
    ...
    if (home && footer && !home.querySelector('.orbitex-faq-section')) {
      ...
      faq.innerHTML = [
        '<div class="orbitex-faq-intro"><div class="orbitex-eyebrow">ORBITEX HELP</div><h2 id="orbitex-faq-title">A few things<br><span>worth knowing.</span></h2><p>Quick answers to the questions traders ask most. Everything you need to get started with ORBITEX.</p><a href="#faq-list" class="orbitex-faq-help">Visit the help center <span>→</span></a></div>',
        '<div class="orbitex-faq-list" id="faq-list">',
        ...
        '<div class="orbitex-faq-item"><button type="button" class="orbitex-faq-question"><span>Where can I get help?</span><span class="orbitex-faq-icon">+</span></button><div class="orbitex-faq-answer"><p>For account or platform questions, use the support options available on ORBITEX. You can also review the relevant guides and help resources before contacting support.</p></div></div>',
        '</div>'
      ].join('');
      footer.parentNode.insertBefore(faq, footer);
      faq.querySelectorAll('.orbitex-faq-question').forEach((button) => button.addEventListener('click', () => {
        const item = button.parentElement;
        if (!item) return;
        const wasOpen = item.classList.contains('is-open');
        faq.querySelectorAll('.orbitex-faq-item.is-open').forEach((openItem) => openItem.classList.remove('is-open'));
        if (!wasOpen) item.classList.add('is-open');
        faq.querySelectorAll('.orbitex-faq-icon').forEach((icon) => {
          icon.textContent = icon.closest('.orbitex-faq-item')?.classList.contains('is-open') ? '×' : '+';
        });
      }));
    }
  ...
  const start = () => { enhancePage(); window.setTimeout(enhancePage, 250); };
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', start, { once: true }); else start();
})();`;

export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  return <html lang="en"><body><script dangerouslySetInnerHTML={{ __html: themeScript }} /><ScrollGuard /><AuthGuard />{children}<MobileNav /><AccountMoreMenu /><script dangerouslySetInnerHTML={{ __html: enhanceScript }} /></body></html>;
}
