...
function CoinLogo({ asset, network }: { asset?: Asset; network?: Network }) {
  ...
  return <span className="deposit-asset-logo" style={{ background: "transparent" }}><img src={`https://assets.coincap.io/assets/icons/${symbol.toLowerCase()}@2x.png`} alt="" onError={(event) => { event.currentTarget.style.display = "none"; const fallback = event.currentTarget.nextElementSibling as HTMLElement | null; if (fallback) fallback.style.display = "grid"; }} /><span style={{ display: "none", background: color, width: "100%", height: "100%", borderRadius: "50%", placeItems: "center" }}>{symbol.slice(0, 2)}</span></span>;
}
...
export default function WithdrawPage() {
  ...
  const chooseAsset = (asset: Asset) => { setSelected(asset); setNetworkId((NETWORKS[asset.symbol] ?? [])[0]?.id ?? ""); setNotice(false); setMobileStep("details"); };
  const submit = () => { if (!address.trim() || !amount || Number(amount) <= 0) return; setNotice(true); };
  return <main className="wallet-page"><header className="wallet-header"><Link href="/wallet" className="wallet-brand"><span className="brand-mark">◉</span> ORBITEX</Link><Link href="/wallet" className="history-link"><ArrowLeft size={18}/> Back to Wallet</Link></header><div className="wallet-content"><div className="wallet-heading"><div><span className="wallet-kicker">MOVE FUNDS OUT</span><h1>Withdraw crypto</h1><p>Choose an asset, select its network, and enter the recipient address.</p></div></div><section className="withdraw-layout"><div className={`wallet-panel withdraw-assets ${mobileStep === "details" ? "mobile-hidden" : ""}`}><div className="panel-top"><div><span className="wallet-kicker">SELECT ASSET</span><h2>Choose a withdrawal asset</h2></div><span className="asset-count">20 supported assets</span></div><div className="asset-search"><Search size={18}/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search by coin name or symbol"/></div><div className="asset-grid">{filtered.map(asset => <button type="button" key={asset.symbol} className={`asset-choice ${selected.symbol === asset.symbol ? "selected" : ""}`} onClick={() => chooseAsset(asset)}><CoinLogo asset={asset}/><span><strong>{asset.symbol}</strong><small>{asset.name}</small></span>{selected.symbol === asset.symbol && <Check size={18}/>}</button>)}</div></div><div className={`wallet-panel withdraw-form ${mobileStep === "assets" ? "mobile-hidden" : ""}`}><button type="button" className="mobile-back-button" onClick={() => setMobileStep("assets")}><ArrowLeft size={17}/> Choose another asset</button><div className="selected-asset"><CoinLogo asset={selected}/><div><span className="wallet-kicker">WITHDRAW</span><h2>{selected.name} <em>{selected.symbol}</em></h2></div></div><div className="field-label">Network <span>Choose the network used by the recipient</span></div><div className="network-grid">{networks.map(item => <button type="button" key={item.id} className={`network-choice ${network?.id === item.id ? "selected" : ""}`} onClick={() => {setNetworkId(item.id); setNotice(false);}}><CoinLogo network={item}/><span><strong>{item.name}</strong><small>{item.short}</small></span>{network?.id === item.id && <Check size={17}/>}</button>)}</div><div className="selected-network-note">You are withdrawing <strong>{selected.symbol}</strong> on <strong>{network?.name} ({network?.short})</strong>.</div><label className="field-label" htmlFor="withdraw-address">Withdraw address</label><input id="withdraw-address" className="wallet-input" value={address} onChange={e => setAddress(e.target.value)} placeholder={`Enter ${selected.symbol} address on ${network?.short ?? "selected network"}`}/><label className="field-label" htmlFor="withdraw-amount">Amount</label><div className="amount-wrap"><input id="withdraw-amount" className="wallet-input" type="number" min="0" value={amount} onChange={e => setAmount(e.target.value)} placeholder={`0.00 ${selected.symbol}`}/><span>{selected.symbol}</span></div><button type="button" className="primary-action" onClick={submit}><ArrowUpFromLine size={18}/> Withdraw {selected.symbol}</button>{notice && <div className="withdraw-notice"><Check size={18}/><span>Withdrawal request preview created. No real funds have been sent.</span></div>}<p className="form-warning"><ShieldCheck size={16}/> Always confirm the coin and network before sending. Using the wrong network may permanently lose funds.</p></div></section></div><MobileNav /></main>;
}
