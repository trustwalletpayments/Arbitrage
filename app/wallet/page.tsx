  ...
  return (
    <main className="wallet-page">
      ...
      <div className="wallet-content">
        ...
        <section className="funding-history-panel" aria-label="Funding history">
          <div className="funding-history-heading"><div><span className="wallet-kicker">FUNDING HISTORY</span><h2>Deposit &amp; withdrawal history</h2><p>Track deposits, withdrawals, and transfers in one place.</p></div><div className="funding-history-actions"><Link href="/wallet/deposit" className="history-deposit"><ArrowDownToLine size={16} /> Deposit</Link><Link href="/wallet/withdraw" className="history-withdraw"><ArrowUpFromLine size={16} /> Withdraw</Link></div></div>
          <div className="funding-history-tabs" role="tablist">{(Object.keys(fundingLabels) as FundingTab[]).map((tab) => <button type="button" role="tab" aria-selected={fundingTab === tab} key={tab} className={fundingTab === tab ? "active" : ""} onClick={() => setFundingTab(tab)}>{fundingLabels[tab]}</button>)}</div>
