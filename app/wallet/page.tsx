    ...
    <main className="wallet-page">
      ...
      <div className="wallet-content">
        ...
        <section className="funding-history-panel">
          <div className="funding-history-heading"><div><span className="wallet-kicker">FUNDING</span><h2>Transaction history</h2><p>Review your deposits, withdrawals, and transfers.</p></div><div className="funding-history-actions"><Link href="/wallet/deposit" className="history-deposit"><ArrowDownToLine size={16} /> Deposit</Link><Link href="/wallet/withdraw" className="history-withdraw"><ArrowUpFromLine size={16} /> Withdraw</Link></div></div>
          <div className="funding-history-tabs">{(Object.keys(fundingLabels) as FundingTab[]).map((tab) => <button type="button" key={tab} className={fundingTab === tab ? "active" : ""} onClick={() => setFundingTab(tab)}>{fundingLabels[tab]}</button>)}</div>
          <div className="funding-history-table"><div className="funding-history-table-head"><span>Date</span><span>Type</span><span>Asset</span><span>Amount</span><span>Status</span><span>Transaction</span></div><div className="funding-history-empty"><div className="funding-empty-icon"><Clock3 size={21} /></div><strong>No transactions yet</strong><span>Your {fundingTab === "all" ? "funding activity" : fundingLabels[fundingTab].toLowerCase()} will appear here.</span></div></div>
        </section>
