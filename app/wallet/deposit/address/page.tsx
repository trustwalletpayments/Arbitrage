...
function DepositAddressContent() {
  ...
  const verificationSupported = (asset === "USDT" && networkId === "bsc") || (asset === "BTC" && networkId === "bitcoin");
  ...
  async function submitDeposit(event: React.FormEvent) {
    ...
    const validTxHash = asset === "BTC" && networkId === "bitcoin"
      ? /^[a-fA-F0-9]{64}$/.test(normalizedHash)
      : /^0x[a-fA-F0-9]{64}$/.test(normalizedHash);
    if (!validTxHash) return setError(asset === "BTC" ? "Enter a valid Bitcoin transaction ID." : "Enter a valid 0x transaction hash.");
    setSubmitting(true);
    const { data: { session } } = await supabase.auth.getSession();
