...
export default function DepositAddressPage(){
  ...
  return <main className="wallet-page"><header className="wallet-header"><Link href="/wallet" className="wallet-brand"><span className="brand-mark">◉</span> ORBITEX</Link><Link href="/wallet/deposit" className="history-link"><ArrowLeft size={18}/> Back to Deposit</Link></header><div className="wallet-content deposit-content"><div className="wallet-heading"><span className="wallet-kicker">FUND YOUR ACCOUNT</span><h1>Deposit {asset}</h1><p>Your deposit address is ready. Send only {asset} through the selected network.</p></div><section className="deposit-address-page-card"><Link href="/wallet/deposit" className="mobile-back-button"><ArrowLeft size={17}/> Choose another network</Link><div className="deposit-address-title"><span className="wallet-kicker">YOUR DEPOSIT ADDRESS</span><h2>{assetName} ({asset})</h2><p>This is your <strong>{asset} deposit address on {network} ({networkShort})</strong>. Only send {asset} using this exact network.</p></div><div className="deposit-qr-placeholder"><QrCode size={92}/><span>QR code will be added here</span></div><div className="deposit-address-box"><span>{asset} on {network} — {networkShort} deposit address</span><strong>{address}</strong><button type="button" onClick={copyAddress}><Copy size={17}/> {copied?"Copied":"Copy address"}</button></div><div className="deposit-security-note"><ShieldCheck size={18}/><span>Network warning: sending another asset or using a different network may permanently result in lost funds.</span></div></section></div><MobileNav/></main>;
}
