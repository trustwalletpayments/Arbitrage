...
export default function DepositPage() {
  ...
  const chooseAsset = (asset: Asset) => { setSelected(asset); setNetworkId(""); setShowNetworks(true); };
  const chooseNetwork = (item: Network) => { setNetworkId(item.id); router.push(`/wallet/deposit/address?asset=${encodeURIComponent(selected.symbol)}&assetName=${encodeURIComponent(selected.name)}&network=${encodeURIComponent(item.name)}&networkShort=${encodeURIComponent(item.short)}&networkId=${encodeURIComponent(item.id)}`); };
