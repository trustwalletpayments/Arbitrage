...
const NETWORKS: Network[] = [
  { id: "ethereum", name: "Ethereum", short: "ERC-20", icon: "ethereum", assets: ["ETH", "USDT", "USDC", "LINK", "UNI"] },
  { id: "bsc", name: "BNB Smart Chain", short: "BEP-20", icon: "bnbchain", assets: ["BNB", "USDT", "USDC", "LINK", "UNI", "POL", "AVAX", "ETH"] },
  { id: "polygon", name: "Polygon", short: "Polygon", icon: "polygon", assets: ["POL", "USDT", "USDC", "ETH", "LINK", "UNI"] },
  { id: "arbitrum", name: "Arbitrum One", short: "Arbitrum", icon: "arbitrum", assets: ["ETH", "USDT", "USDC", "ARB", "LINK", "UNI"] },
  { id: "optimism", name: "Optimism", short: "Optimism", icon: "optimism", assets: ["ETH", "USDT", "USDC", "LINK", "UNI"] },
  { id: "base", name: "Base", short: "Base", icon: "base", assets: ["ETH", "USDT", "USDC", "LINK", "UNI"] },
  { id: "avalanche", name: "Avalanche C-Chain", short: "C-Chain", icon: "avalanche", assets: ["AVAX", "USDT", "USDC", "LINK", "UNI"] },
  { id: "fantom", name: "Fantom", short: "Fantom", icon: "fantom", assets: ["FTM", "USDT", "USDC", "ETH"] },
  { id: "cronos", name: "Cronos", short: "Cronos", icon: "cronos", assets: ["CRO", "USDT", "USDC", "ETH"] },
  { id: "linea", name: "Linea", short: "Linea", icon: "linea", assets: ["ETH", "USDT", "USDC"] },
...
export default function DepositPage() {
  ...
  const routeKeys = useMemo(() => new Set(supportedRoutes.filter((route) => route.ready).map((route) => `${route.asset}:${route.network}`)), [supportedRoutes]);
  ...
  useEffect(() => {
    ...
    async function loadSupportedRoutes() {
      ...
      try {
        ...
        if (!cancelled && response.ok && Array.isArray(result.routes)) setSupportedRoutes(result.routes);
      } finally {
  ...
  const isEnabled = (asset: string, network: string) => routeKeys.has(`${asset}:${network}`);
  ...
  return (
    <main className="wallet-page">
      ...
      <div className="wallet-content deposit-content">
        <div className="wallet-heading"><div><span className="wallet-kicker">FUND YOUR ACCOUNT</span><h1>Deposit</h1><p>{showNetworks ? `Choose a network for ${selectedAsset.symbol}.` : "Select the asset you want to deposit."}</p></div></div>
        <section className="deposit-shell">
          <div className={`deposit-selector withdraw-assets ${showNetworks ? "mobile-hidden" : ""}`}>
            ...
            <p className="deposit-helper">{loadingRoutes ? "Loading available deposit networks…" : `${supportedRoutes.filter((route) => route.ready).length} configured EVM deposit routes are available. Networks without a configured token contract remain disabled to prevent unsupported deposits.`}</p>
          </div>
